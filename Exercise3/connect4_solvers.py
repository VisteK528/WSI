...
class MinMaxSolver:
    def __init__(self, game: ConnectFour, row_count: int, column_count: int, max_player, min_player):
        self.game = game
    ...
    def get_best_move_2(self, depth: int) -> int:
        col, score = self.minimax_prune(self.game.state, depth, -np.inf, np.inf,
                                        True)
        return col
        ...
        if is_terminal or depth == 0:
            if is_terminal:
                ...
                if is_maximizing_player:
                    return (None, self.evaluate_position(board, self._max_player))
                else:
                    return (None, self.evaluate_position(board, self._min_player))

        if is_maximizing_player:
            value = -np.inf
            chosen_column = choice(valid_moves)
            for valid_move in valid_moves:
                evaluation = self.minimax_prune(simulate_move(board, valid_move), depth - 1, alpha, beta, False)[1]
            ...
            chosen_column = choice(valid_moves)
            for valid_move in valid_moves:
                evaluation = self.minimax_prune(simulate_move(board, valid_move), depth - 1, alpha, beta, True)[1]
